...
int main(int argc, char **argv)
{
    std::string data_type, dist_fn, data_path, index_path_prefix, codebook_prefix, label_file, universal_label,
        label_type;
    uint32_t num_threads, R, L, disk_PQ, build_PQ, QD, Lf, filter_threshold, num_shard, num_parts;
    float B, M;
    ...
    try
    {
        ...
        optional_configs.add_options()("shard_num", po::value<uint32_t>(&num_shard)->default_value(-1),
                                       "The shard to build");
        optional_configs.add_options()("num_parts", po::value<uint32_t>(&num_parts)->default_value(-1),
                                       "The number of shards we have to merge");
        // optional_configs.add_options()("split", po::bool_switch()->default_value(false),
        //                                "Whether to just split the data into shards if it cannot be hold in the current memory budgets"),
        //                                program_options_utils::SPLIT);
        optional_configs.add_options()("split", po::value<bool>(&split)->default_value(false),
                                       "Whether to just split the data into shards if it cannot be hold in the current memory budgets");
    ...
    std::string params;
    // modified: the shards function 
    // custom parameters
    if (num_parts != -1)  { // if merging 
        params = std::string(std::to_string(R)) + " " + std::string(std::to_string(L)) + " " +
                        std::string(std::to_string(num_parts)) + " " + 
    ...
    }  else if (num_shard != -1) { // if building a shard
        params = std::string(std::to_string(R)) + " " + std::string(std::to_string(L)) + " " +
                        std::string(std::to_string(num_shard)) + " " + 
                        std::string(std::to_string(B)) + " " + std::string(std::to_string(M)) + " " +
                        std::string(std::to_string(num_threads)) + " " + std::string(std::to_string(disk_PQ)) + " " +
                        std::string(std::to_string(append_reorder_data)) + " " +
                        std::string(std::to_string(build_PQ)) + " " + std::string(std::to_string(QD)); 
    ...
    try
    {
        ...
        {
            if (split) { // splitting into shards
                std::cout << "Just splitting the shards with labels" << std::endl;
                if (data_type == std::string("int8")) 
                    return diskann::split_to_shards<int8_t>(data_path.c_str(), index_path_prefix.c_str(), params.c_str(),
                                                            metric, use_opq, codebook_prefix, use_filters, label_file,
                                                            universal_label, filter_threshold, Lf);
                else if (data_type == std::string("uint8"))
                    return diskann::split_to_shards<uint8_t, uint16_t>(
                    ...
                    return diskann::split_to_shards<float, uint16_t>(
                        data_path.c_str(), index_path_prefix.c_str(), params.c_str(), metric, use_opq, codebook_prefix,
                        use_filters, label_file, universal_label, filter_threshold, Lf);
                else
                {
            ...
            } else if (num_parts != -1) { // merging shards

                std::cout << "Just merging the shards with labels" << std::endl;
                if (data_type == std::string("int8")) 
                    return diskann::custom_merge<int8_t>(data_path.c_str(), index_path_prefix.c_str(), params.c_str(),
                                                            metric, use_opq, codebook_prefix, use_filters, label_file,
                                                            universal_label, filter_threshold, Lf);
                else if (data_type == std::string("uint8"))
                    return diskann::custom_merge<uint8_t, uint16_t>(
                        data_path.c_str(), index_path_prefix.c_str(), params.c_str(), metric, use_opq, codebook_prefix,
                        use_filters, label_file, universal_label, filter_threshold, Lf);
                else if (data_type == std::string("float"))
                    return diskann::custom_merge<float, uint16_t>(
                        data_path.c_str(), index_path_prefix.c_str(), params.c_str(), metric, use_opq, codebook_prefix,
                        use_filters, label_file, universal_label, filter_threshold, Lf);
                else
                {
            ...
            } else if (num_shard != -1) { // build a single shard

                std::cout << "Just build a single shard with labels" << std::endl;
                if (data_type == std::string("int8")) 
                    return diskann::one_shard<int8_t>(data_path.c_str(), index_path_prefix.c_str(), params.c_str(),
                                                            metric, use_opq, codebook_prefix, use_filters, label_file,
                                                            universal_label, filter_threshold, Lf);
                else if (data_type == std::string("uint8"))
                    return diskann::one_shard<uint8_t, uint16_t>(
                        data_path.c_str(), index_path_prefix.c_str(), params.c_str(), metric, use_opq, codebook_prefix,
                        use_filters, label_file, universal_label, filter_threshold, Lf);
                else if (data_type == std::string("float"))
                    return diskann::one_shard<float, uint16_t>(
                        data_path.c_str(), index_path_prefix.c_str(), params.c_str(), metric, use_opq, codebook_prefix,
                        use_filters, label_file, universal_label, filter_threshold, Lf);
                else
                {
        ...
        {     
            if (split) { // splitting shards

                std::cout << "Just splitting the shards " << std::endl;
                if (data_type == std::string("int8")) {
                    return diskann::split_to_shards<int8_t>(data_path.c_str(), index_path_prefix.c_str(), params.c_str(),
                                                         metric, use_opq, codebook_prefix, use_filters, label_file,
                                                         universal_label, filter_threshold, Lf);
                    ...
                    return diskann::split_to_shards<uint8_t>(data_path.c_str(), index_path_prefix.c_str(), params.c_str(),
                                                            metric, use_opq, codebook_prefix, use_filters, 
                                                            label_file, universal_label, 
                                                            filter_threshold, Lf);
                else if (data_type == std::string("float"))
                    return diskann::split_to_shards<float>(data_path.c_str(), index_path_prefix.c_str(), params.c_str(),
                                                            metric, use_opq, codebook_prefix, use_filters, 
                                                            label_file, universal_label, 
                                                            filter_threshold, Lf);
                ...
                std::cout << "Just merging the shards " << std::endl;
                if (data_type == std::string("int8")) {
                    return diskann::custom_merge<int8_t>(data_path.c_str(), index_path_prefix.c_str(), params.c_str(),
                                                         metric, use_opq, codebook_prefix, use_filters, label_file,
                                                         universal_label, filter_threshold, Lf);
                    ...
                    return diskann::custom_merge<uint8_t>(data_path.c_str(), index_path_prefix.c_str(), params.c_str(),
                                                            metric, use_opq, codebook_prefix, use_filters, 
                                                            label_file, universal_label, 
                                                            filter_threshold, Lf);
                else if (data_type == std::string("float"))
                    return diskann::custom_merge<float>(data_path.c_str(), index_path_prefix.c_str(), params.c_str(),
                                                            metric, use_opq, codebook_prefix, use_filters, 
                                                            label_file, universal_label, 
                                                            filter_threshold, Lf);
                else
            ...
            } else if (num_shard != -1) { // building a shard

                std::cout << "Just build a shard: " << num_shard << std::endl;
                if (data_type == std::string("int8")) {
                    return diskann::one_shard<int8_t>(data_path.c_str(), index_path_prefix.c_str(), params.c_str(),
                                                         metric, use_opq, codebook_prefix, use_filters, label_file,
                                                         universal_label, filter_threshold, Lf);
                    ...
                    return diskann::one_shard<uint8_t>(data_path.c_str(), index_path_prefix.c_str(), params.c_str(),
                                                            metric, use_opq, codebook_prefix, use_filters, 
                                                            label_file, universal_label, 
                                                            filter_threshold, Lf);
                else if (data_type == std::string("float"))
                    return diskann::one_shard<float>(data_path.c_str(), index_path_prefix.c_str(), params.c_str(),
                                                            metric, use_opq, codebook_prefix, use_filters, 
                                                            label_file, universal_label, 
                                                            filter_threshold, Lf);
                else
            ...
            } else { // ordinary build_disk_index

                if (data_type == std::string("int8")) {
                    
                    diskann::cout << "calling build_disk_index<int8_t>" << std::endl;
                    return diskann::build_disk_index<int8_t>(data_path.c_str(), index_path_prefix.c_str(), params.c_str(),
                                                            metric, use_opq, codebook_prefix, use_filters, label_file,
                                                            universal_label, filter_threshold, Lf);
                }
