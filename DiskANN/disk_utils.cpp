                 ...
                 const std::string &labels_to_medoids_file)
{
    ...
    diskann::cout << "Entering Merge Shards" << std::endl;
                              ...
                              const uint32_t Lf)
{
    ...
    diskann::cout << "basefile has #points: " << base_num << "; #dims: " << base_dim << std::endl; 
    ...
    for (int p = 0; p < num_parts; p++)
    {
        ...
        diskann::cout << "shard_base_pts: " << shard_base_pts << "; shard_base_dim: " << shard_base_dim << std::endl; 
...
template <typename T, typename LabelT>
int split_to_shards(const char *dataFilePath, const char *indexFilePath, const char *indexBuildParameters,
                     ...
                     const uint32_t Lf) 
{
    ...
    diskann::cout << "Splitting the whole vector into shards..." << std::endl;

    // Original Set up Parameters
...
// This code should only be available in the "build" DLL.
    ...
    // sampling_rate p_val
    // ram_budget = indexing_ram_budget
    // medoids_file = medoids_path
    // centroids_file = centroids_path
    // label_file = labels_file_to_use
    // labels_to_medoids_file = labels_to_medoids_path

    size_t base_num, base_dim;
    // use the correct file -> in case for mips
    diskann::get_bin_metadata(data_file_to_use, base_num, base_dim);
    // estimate ram budget
    double full_index_ram = estimate_ram_usage(base_num, (uint32_t)base_dim, sizeof(T), R);

    // if possible, just build in one-shot
    if (full_index_ram < indexing_ram_budget * 1024 * 1024 * 1024)
    {
        ...
        diskann::Index<T, TagT, LabelT> _index(compareMetric, base_dim, base_num,
                                               ...
                                               build_pq_bytes > 0, build_pq_bytes, use_opq, use_filters);
        if (!use_filters)
            _index.build(data_file_to_use.c_str(), base_num);
        else
        {
            if (universal_label != "")
            { //  indicates no universal label
            ...
            _index.build_filtered_index(data_file_to_use.c_str(), labels_file_to_use, base_num);
        }
        _index.save(mem_index_path.c_str());
        ...
        if (use_filters)
        {
            // need to copy the labels_to_medoids file to the specified input
            // file
            std::remove(labels_to_medoids_path.c_str());
            std::string mem_labels_to_medoid_file = mem_index_path + "_labels_to_medoids.txt";
            copy_file(mem_labels_to_medoid_file, labels_to_medoids_path);
            std::remove(mem_labels_to_medoid_file.c_str());
        }

        std::remove(medoids_path.c_str());
        std::remove(centroids_path.c_str());
        return 0;
    }
    ...
    timer.reset();
    int num_parts =
        partition_with_ram_budget<T>(data_file_to_use, p_val, indexing_ram_budget, 2 * R / 3, merged_index_prefix, 2);
    diskann::cout << timer.elapsed_seconds_for_step("partitioning data ") << std::endl;
...
template <typename T, typename LabelT>
int one_shard(const char *dataFilePath, const char *indexFilePath, const char *indexBuildParameters,
                     diskann::Metric compareMetric, bool use_opq, const std::string &codebook_prefix, bool use_filters,
                     const std::string &label_file, const std::string &universal_label, const uint32_t filter_threshold,
                     const uint32_t Lf)
{ // no filter support

    diskann::cout << "You just want to build a shard!" << std::endl;
    // process the file names
    ...
    if (param_list.size() < 6 || param_list.size() > 10)
    {
        diskann::cout << "Wrong usage. " << std::endl;
        return -1;
    }
    ...
    // if there is a 7th parameter, it means we compress the disk index
    // vectors also using PQ data (for very large dimensionality data). If the
    // provided parameter is 0, it means we store full vectors.
    if (param_list.size() > 6)
    {
        disk_pq_dims = atoi(param_list[6].c_str());
        use_disk_pq = true;
        if (disk_pq_dims == 0)
            use_disk_pq = false;
    }
    ...
    {
        build_pq_bytes = atoi(param_list[8].c_str()); // needed 
    }

    uint32_t R = (uint32_t)atoi(param_list[0].c_str());
    uint32_t L = (uint32_t)atoi(param_list[1].c_str());
    uint32_t shard_number = (uint32_t)atoi(param_list[2].c_str());

    std::string base_file(dataFilePath); // needed
    std::string data_file_to_use = base_file; // need for calculating sampling rate 
    std::string index_prefix_path(indexFilePath);
    std::string label_file_to_use = index_prefix_path + "_label_formatted.txt"; // needed 
    // std::string pq_pivots_path_base = codebook_prefix;
    ...
    // std::string pq_compressed_vectors_path = index_prefix_path + "_pq_compressed.bin";
    std::string mem_index_path = index_prefix_path + "_mem.index"; // needed 
    std::string disk_index_path = index_prefix_path + "_disk.index"; // nn
    std::string medoids_file = disk_index_path + "_medoids.bin"; // needed 
    std::string centroids_file = disk_index_path + "_centroids.bin"; // needed 

    std::string labels_to_medoids_file = disk_index_path + "_labels_to_medoids.txt"; // needed 
    // std::string mem_labels_file = mem_index_path + "_labels.txt";
    // std::string disk_labels_file = disk_index_path + "_labels.txt";
    // std::string mem_univ_label_file = mem_index_path + "_universal_label.txt";
    // std::string disk_univ_label_file = disk_index_path + "_universal_label.txt";
    ...
    // ### map the names as in build_merged vamana_index ###
    ...
    // sampling rate <- p_val
    size_t points_num, dim;
    diskann::get_bin_metadata(data_file_to_use.c_str(), points_num, dim); // !!! dim here is old
    diskann::cout << "data_file_to_use has points_num: " << points_num << "; dim: " << dim << std::endl; 
    const double sampling_rate = ((double)MAX_PQ_TRAINING_SET_SIZE / (double)points_num);


    // ram_budget <- indexing_ram_budget 
    double indexing_ram_budget = (float)atof(param_list[4].c_str());
    if (indexing_ram_budget <= 0)
    {
        std::cerr << "Not building index. Please provide more RAM budget" << std::endl;
        return -1;
    ...
    // num_threads
    uint32_t num_threads = (uint32_t)atoi(param_list[5].c_str());
    ...
    diskann::cout << "Starting index build: R=" << R << " L=" << L 
                  ...
                  << " on shard: " << shard_number << std::endl;
    ...
    std::string merged_index_prefix = mem_index_path + "_tempFiles"; // dependency good 

    // where the universal label is to be saved in the final graph
    std::string final_index_universal_label_file = mem_index_path + "_universal_label.txt";

    std::string shard_base_file = merged_index_prefix + "_subshard-" + std::to_string(shard_number) + ".bin"; // good

    std::string shard_ids_file = merged_index_prefix + "_subshard-" + std::to_string(shard_number) + "_ids_uint32.bin"; // good

    std::string shard_labels_file = merged_index_prefix + "_subshard-" + std::to_string(shard_number) + "_labels.txt"; // good

    retrieve_shard_data_from_ids<T>(data_file_to_use, shard_ids_file, shard_base_file); // good

    std::string shard_index_file = merged_index_prefix + "_subshard-" + std::to_string(shard_number) + "_mem.index"; // -> shard index // good
    ...
    diskann::IndexWriteParameters low_degree_params = diskann::IndexWriteParametersBuilder(L, 2 * R / 3)
                                                            ...
                                                            .with_num_threads(num_threads)
                                                            .build(); // original R
    // degree of subgraph is lower here -> 2/3
    uint64_t shard_base_dim, shard_base_pts;
    get_bin_metadata(shard_base_file, shard_base_pts, shard_base_dim); // read the bin meta data of the shard // good
    diskann::cout << "shard_base_pts: " << shard_base_pts << "; shard_base_dim: " << shard_base_dim << std::endl; 

    // always use L2 here as we've used the prepped data base file
    compareMetric = diskann::Metric::L2; 
    ...
    {
        diskann::extract_shard_labels(label_file_to_use, shard_ids_file, shard_labels_file);
        if (universal_label != "")
        { //  indicates no universal label
            LabelT unv_label_as_num = 0;
            _index.set_universal_label(unv_label_as_num);
...
template <typename T, typename LabelT>
int custom_merge(const char *dataFilePath, const char *indexFilePath, const char *indexBuildParameters,
                     diskann::Metric compareMetric, bool use_opq, const std::string &codebook_prefix, bool use_filters,
                     const std::string &label_file, const std::string &universal_label, const uint32_t filter_threshold,
                     const uint32_t Lf)
{
    ...
    uint32_t R = (uint32_t)atoi(param_list[0].c_str()); // !!! R might be wrong 
    uint32_t num_parts = (uint32_t)atoi(param_list[2].c_str());

    std::string base_file(dataFilePath); // needed
    std::string data_file_to_use = base_file; // need for calculating sampling rate 
    std::string labels_file_original = label_file; // copy
    std::string index_prefix_path(indexFilePath);
    std::string labels_file_to_use = index_prefix_path + "_label_formatted.txt"; // needed 
    // std::string pq_pivots_path_base = codebook_prefix;
    // std::string pq_pivots_path = file_exists(pq_pivots_path_base) ? pq_pivots_path_base + "_pq_pivots.bin" : index_prefix_path + "_pq_pivots.bin";
    std::string pq_compressed_vectors_path = index_prefix_path + "_pq_compressed.bin";
    std::string mem_index_path = index_prefix_path + "_mem.index"; // needed 
    ...
    std::string dummy_remap_file = disk_index_path + "_dummy_remap.txt"; // remap will be used if we break-up points of hig

    // in case if mips -> use the correct augmented file
    if (compareMetric == diskann::Metric::INNER_PRODUCT)
    {
        std::string prepped_base = index_prefix_path + "_prepped_base.bin";
        data_file_to_use = prepped_base;
    ...
    // end of merging files

    // ### back to build diskindex clean up ###
    timer.reset();
    if (!use_disk_pq)
    {
    ...
    return 0; //safely close the program 
                     ...
                     const uint32_t Lf)
{
    ...
    for (uint64_t i = 0; i < param_list.size(); i++) { //
        diskann::cout << param_list[i].c_str();  // 
    } //
...
// one_shard turns
template DISKANN_DLLEXPORT int one_shard<int8_t, uint32_t>(const char *dataFilePath, const char *indexFilePath,
                                                        const char *indexBuildParameters,
                                                        diskann::Metric compareMetric, bool use_opq,
                                                        const std::string &codebook_prefix, bool use_filters,
                                                        const std::string &label_file,
                                                        const std::string &universal_label,
                                                        const uint32_t filter_threshold, const uint32_t Lf);
template DISKANN_DLLEXPORT int one_shard<uint8_t, uint32_t>(const char *dataFilePath, const char *indexFilePath,
                                                        const char *indexBuildParameters,
                                                        diskann::Metric compareMetric, bool use_opq,
                                                        const std::string &codebook_prefix, bool use_filters,
                                                        const std::string &label_file,
                                                        const std::string &universal_label,
                                                        const uint32_t filter_threshold, const uint32_t Lf);
template DISKANN_DLLEXPORT int one_shard<float, uint32_t>(const char *dataFilePath, const char *indexFilePath,
                                                        const char *indexBuildParameters,
                                                        diskann::Metric compareMetric, bool use_opq,
                                                        const std::string &codebook_prefix, bool use_filters,
                                                        const std::string &label_file,
                                                        ...
                                                        const uint32_t filter_threshold, const uint32_t Lf);
// LabelT = uint16
template DISKANN_DLLEXPORT int one_shard<int8_t, uint16_t>(const char *dataFilePath, const char *indexFilePath,
                                                        const char *indexBuildParameters,
                                                        diskann::Metric compareMetric, bool use_opq,
                                                        const std::string &codebook_prefix, bool use_filters,
                                                        const std::string &label_file,
                                                        const std::string &universal_label,
                                                        const uint32_t filter_threshold, const uint32_t Lf);
template DISKANN_DLLEXPORT int one_shard<uint8_t, uint16_t>(const char *dataFilePath, const char *indexFilePath,
                                                        const char *indexBuildParameters,
                                                        diskann::Metric compareMetric, bool use_opq,
                                                        const std::string &codebook_prefix, bool use_filters,
                                                        const std::string &label_file,
                                                        const std::string &universal_label,
                                                        const uint32_t filter_threshold, const uint32_t Lf);
template DISKANN_DLLEXPORT int one_shard<float, uint16_t>(const char *dataFilePath, const char *indexFilePath,
                                                        const char *indexBuildParameters,
                                                        diskann::Metric compareMetric, bool use_opq,
                                                        const std::string &codebook_prefix, bool use_filters,
                                                        const std::string &label_file,
...
// custom_merge turns
template DISKANN_DLLEXPORT int custom_merge<int8_t, uint32_t>(const char *dataFilePath, const char *indexFilePath,
                                                        const char *indexBuildParameters,
                                                        diskann::Metric compareMetric, bool use_opq,
                                                        const std::string &codebook_prefix, bool use_filters,
                                                        const std::string &label_file,
                                                        const std::string &universal_label,
                                                        const uint32_t filter_threshold, const uint32_t Lf);
template DISKANN_DLLEXPORT int custom_merge<uint8_t, uint32_t>(const char *dataFilePath, const char *indexFilePath,
                                                        const char *indexBuildParameters,
                                                        diskann::Metric compareMetric, bool use_opq,
                                                        const std::string &codebook_prefix, bool use_filters,
                                                        const std::string &label_file,
                                                        const std::string &universal_label,
                                                        const uint32_t filter_threshold, const uint32_t Lf);
template DISKANN_DLLEXPORT int custom_merge<float, uint32_t>(const char *dataFilePath, const char *indexFilePath,
                                                        const char *indexBuildParameters,
                                                        diskann::Metric compareMetric, bool use_opq,
                                                        const std::string &codebook_prefix, bool use_filters,
                                                        const std::string &label_file,
                                                        ...
                                                        const uint32_t filter_threshold, const uint32_t Lf);
// LabelT = uint16
template DISKANN_DLLEXPORT int custom_merge<int8_t, uint16_t>(const char *dataFilePath, const char *indexFilePath,
                                                        const char *indexBuildParameters,
                                                        diskann::Metric compareMetric, bool use_opq,
                                                        const std::string &codebook_prefix, bool use_filters,
                                                        const std::string &label_file,
                                                        const std::string &universal_label,
                                                        const uint32_t filter_threshold, const uint32_t Lf);
template DISKANN_DLLEXPORT int custom_merge<uint8_t, uint16_t>(const char *dataFilePath, const char *indexFilePath,
                                                        const char *indexBuildParameters,
                                                        diskann::Metric compareMetric, bool use_opq,
                                                        const std::string &codebook_prefix, bool use_filters,
                                                        const std::string &label_file,
                                                        const std::string &universal_label,
                                                        const uint32_t filter_threshold, const uint32_t Lf);
template DISKANN_DLLEXPORT int custom_merge<float, uint16_t>(const char *dataFilePath, const char *indexFilePath,
                                                        const char *indexBuildParameters,
                                                        diskann::Metric compareMetric, bool use_opq,
                                                        const std::string &codebook_prefix, bool use_filters,
                                                        const std::string &label_file,
...
// spilt into shards 
template DISKANN_DLLEXPORT int split_to_shards<int8_t, uint32_t>(const char *dataFilePath, const char *indexFilePath,
                                                        const char *indexBuildParameters,
                                                        diskann::Metric compareMetric, bool use_opq,
                                                        const std::string &codebook_prefix, bool use_filters,
                                                        const std::string &label_file,
                                                        const std::string &universal_label,
                                                        const uint32_t filter_threshold, const uint32_t Lf);
template DISKANN_DLLEXPORT int split_to_shards<uint8_t, uint32_t>(const char *dataFilePath, const char *indexFilePath,
                                                        const char *indexBuildParameters,
                                                        diskann::Metric compareMetric, bool use_opq,
                                                        const std::string &codebook_prefix, bool use_filters,
                                                        const std::string &label_file,
                                                        const std::string &universal_label,
                                                        const uint32_t filter_threshold, const uint32_t Lf);
template DISKANN_DLLEXPORT int split_to_shards<float, uint32_t>(const char *dataFilePath, const char *indexFilePath,
                                                        const char *indexBuildParameters,
                                                        diskann::Metric compareMetric, bool use_opq,
                                                        const std::string &codebook_prefix, bool use_filters,
                                                        const std::string &label_file,
                                                        ...
                                                        const uint32_t filter_threshold, const uint32_t Lf);
// LabelT = uint16
template DISKANN_DLLEXPORT int split_to_shards<int8_t, uint16_t>(const char *dataFilePath, const char *indexFilePath,
                                                        const char *indexBuildParameters,
                                                        diskann::Metric compareMetric, bool use_opq,
                                                        const std::string &codebook_prefix, bool use_filters,
                                                        const std::string &label_file,
                                                        const std::string &universal_label,
                                                        const uint32_t filter_threshold, const uint32_t Lf);
template DISKANN_DLLEXPORT int split_to_shards<uint8_t, uint16_t>(const char *dataFilePath, const char *indexFilePath,
                                                        const char *indexBuildParameters,
                                                        diskann::Metric compareMetric, bool use_opq,
                                                        const std::string &codebook_prefix, bool use_filters,
                                                        const std::string &label_file,
                                                        const std::string &universal_label,
                                                        const uint32_t filter_threshold, const uint32_t Lf);
template DISKANN_DLLEXPORT int split_to_shards<float, uint16_t>(const char *dataFilePath, const char *indexFilePath,
                                                        const char *indexBuildParameters,
                                                        diskann::Metric compareMetric, bool use_opq,
                                                        const std::string &codebook_prefix, bool use_filters,
                                                        const std::string &label_file,
